...
class AddCreditToCustomerScreen extends StatefulWidget {

  @override
  State<AddCreditToCustomerScreen> createState() => _AddCreditToCustomerScreenState();
}

class _AddCreditToCustomerScreenState extends State<AddCreditToCustomerScreen> {
  @override
      ...
      builder: (context, state) {
        ...
        return Scaffold(
          appBar: AppBar(
            ...
            centerTitle: true,
            title: const Text('Update Customer Credit',
              ...
              child: Column(
                children: [
                    ...
                    children: [
                      Expanded(
                        child: SearchBarBlock(
                            controller: TextEditingController(), function: () async {
                          await  profCubit.getCreditUserData(context: context);
                          profCubit.searchUserEmailController.clear();
                        }),
                      ),InkWell(
                  ...
                  GeneralTextFieldBlock(hint: 'current credit', controller: profCubit.currentCreditController!,canEdit: false,),
                    const SizedBox(height: 10,),
                    GeneralTextFieldBlock(hint: 'new total credit', controller: profCubit.newCreditController,onlyInteger: true,),
                    const SizedBox(height: 10,),
                    GeneralTextFieldBlock(hint: 'start credit date', controller:profCubit.startCreditController,canEdit: false,suffixIcon: const Icon(Icons.timer_outlined),isStartTime: true),
                    const SizedBox(height: 10,),
                    GeneralTextFieldBlock(hint: 'end credit date', controller:profCubit.endCreditController,canEdit: false,suffixIcon: const Icon(Icons.timer_off_outlined),isStartTime: false,),
                    const SizedBox(height: 10,),
                    GeneralButtonBlock(
